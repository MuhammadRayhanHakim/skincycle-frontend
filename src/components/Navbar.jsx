...
import {
  Bell, ShoppingCart, User, Settings, LogOut, MessageSquare,
  Heart, RefreshCw, ChevronDown, BookOpen, FileText, Recycle, Package, History,
} from "lucide-react";
  ...
  const fetchCartCount = async () => {
    ...
    try {
      ...
      if (data.status === "success") {
        const total = data.data?.reduce((sum, item) => sum + (item.kuantitas || 1), 0) || 0;
        setCartCount(total);
      }
    } catch (error) {
  ...
  const handleToggleNotif = async () => {
    ...
    if (nextShowState && notifCount > 0) {
      ...
      try {
        ...
        if (resData.status === "success") setNotifCount(0);
      } catch (error) {
    ...
    <>
      <nav className="fixed top-0 left-0 w-full h-16 flex justify-between items-center px-10 bg-white/90 backdrop-blur-md shadow-sm border-b border-brand-primary-300/5 font-sans z-50">
        <div className="flex items-center gap-12">
          <Link to="/" className="flex items-center h-full transition-transform hover:scale-105">
            <img src="/logo.png" alt="SkinCycle Logo" className="h-14 w-auto object-contain" />
          </Link>
          ...
          <div className="hidden md:flex gap-1 items-center">
            <NavLink
              ...
              className={({ isActive }) =>
                `px-4 py-1.5 rounded-lg text-[13px] font-bold transition-all ${
                  isActive ? "bg-brand-primary-300 text-neutral-default shadow-sm" : "text-brand-primary-300 hover:bg-brand-primary-100/20"
                }`
              }
            >
              Beranda
            ...
            <div className="relative group">
              ...
              <div className="absolute left-0 top-full pt-2 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all pointer-events-none group-hover:pointer-events-auto z-50">
                <div className="bg-neutral-default shadow-2xl rounded-2xl p-2 border border-neutral-100 min-w-[200px]">
                  <Link to="/ensiklopedia/kumpulan" className="w-full text-left px-4 py-2.5 text-[12px] font-bold text-neutral-600 hover:bg-neutral-50 rounded-xl flex items-center gap-2 transition-colors">
                    <FileText className="w-3.5 h-3.5 text-brand-primary-300" /> Artikel Edukasi
                  </Link>
                  <Link to="/ensiklopedia/kandungan" className="w-full text-left px-4 py-2.5 text-[12px] font-bold text-neutral-600 hover:bg-neutral-50 rounded-xl flex items-center gap-2 transition-colors">
                    <BookOpen className="w-3.5 h-3.5 text-brand-primary-300" /> Kandungan Bahan
                  </Link>
                </div>
              </div>
            </div>
            ...
            <div className="relative group">
              ...
              <div className="absolute left-0 top-full pt-2 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all pointer-events-none group-hover:pointer-events-auto z-50">
                <div className="bg-neutral-default shadow-2xl rounded-2xl p-2 border border-neutral-100 min-w-[200px]">
                  <Link to="/daur-ulang" className="w-full text-left px-4 py-2.5 text-[12px] font-bold text-neutral-600 hover:bg-neutral-50 rounded-xl flex items-center gap-2 transition-colors">
                    <Recycle className="w-3.5 h-3.5 text-brand-primary-300" /> Edukasi Daur Ulang
                  </Link>
                  <Link to="/daur-ulang/simpan" className="w-full text-left px-4 py-2.5 text-[12px] font-bold text-neutral-600 hover:bg-neutral-50 rounded-xl flex items-center gap-2 transition-colors">
                    <Package className="w-3.5 h-3.5 text-brand-primary-300" /> Mulai Setor Sampah
                  </Link>
                  <Link to="/riwayat" className="w-full text-left px-4 py-2.5 text-[12px] font-bold text-neutral-600 hover:bg-neutral-50 rounded-xl flex items-center gap-2 transition-colors">
                    <History className="w-3.5 h-3.5 text-brand-primary-300" /> Riwayat &amp; Saldo
                  </Link>
                </div>
              </div>
            </div>
                    ...
                    <div className="max-h-80 overflow-y-auto">
                      {notifications.length > 0 ? (
                        notifications.map((notif) => {
                          ...
                          if (notif.tipe === "suka" || notif.tipe === "like") {
                          ...
                          } else if (notif.tipe === "balasan" || notif.id_komentar) {
                            ...
                            if (user.role === "admin") {
                              teksAksi = `Permintaan penjemputan baru dari ${notif.pengirim?.username || "user"} di wilayah Bekasi Regency.`;
                            } else {
                              if (notif.tipe === "saldo_cair") {
                                teksAksi = "telah mengirimkan sampah ke pengepul dan saldo sudah dikirim ke anda";
                              } else {
                                teksAksi = "sampah anda sedang dalam penjemputan";
                              }
                            }
                          } else {
                            teksAksi = notif.pesan || "melakukan interaksi pada akun anda";
                          }

                          if (notif.id_laporan && teksAksi === "melakukan interaksi pada akun anda") return null;

                          const pesanDatabase = notif.pesan || "";
                          const kataKunciSukuBiasa = pesanDatabase.split(" ");
                          const namaUserPengirim = kataKunciSukuBiasa[0] || "";
                          const sisaTeksKalimatAksi = kataKunciSukuBiasa.slice(1).join(" ");

                          return (
                            <div
                              key={notif.id_notifikasi}
                              onClick={() => handleNotifClick(notif)}
                              className={`p-4 border-b border-neutral-50 hover:bg-neutral-50 transition-all cursor-pointer flex items-center gap-3 ${!notif.is_read ? "bg-brand-primary-100/10" : ""}`}
                            >
                              ...
                              <div className="flex flex-col">
                                <p className="text-[11px] text-neutral-700 leading-tight">
                                  {notif.pesan ? (
                                    <>
                                      <span className="font-black text-brand-primary-300">{namaUserPengirim}</span>{" "}
                                      <span>{sisaTeksKalimatAksi}</span>
                                    </>
                                  ) : (
                                    <>
                                      <span className="font-black text-brand-primary-300">
                                        {user.role === "admin" ? notif.pengirim?.username || "User" : notif.pengirim?.username || "Seseorang"}
                                      </span>{" "}
                                      {teksAksi}
                                    </>
                                  )}
                        ...
                        <div className="p-10 text-center">
                          <p className="text-[10px] text-neutral-400 italic">Tidak ada pemberitahuan baru.</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="absolute right-0 top-full pt-3 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all pointer-events-none group-hover:pointer-events-auto z-50">
                    <div className="bg-neutral-default shadow-2xl rounded-2xl p-2 border border-neutral-100 min-w-[170px]">
                        ...
                        <NavLink
                          ...
                          className={({ isActive }) =>
                            `w-full text-left px-4 py-2 text-[11px] font-bold rounded-xl uppercase flex items-center gap-2 mb-1 transition-colors ${
                              isActive ? "bg-[#3D5532] text-white hover:bg-[#3D5532]" : "text-neutral-600 hover:bg-neutral-50"
                            }`
                          }
                        >
                          {({ isActive }) => (
                            <>
                              <Settings className={`w-3.5 h-3.5 ${isActive ? "text-white" : "text-brand-primary-300"}`} />
                      ...
                      >
                        {({ isActive }) => (
                          <>
                            <User className={`w-3.5 h-3.5 ${isActive ? "text-white" : "text-brand-primary-300"}`} />
                            <span>Akun Saya</span>
                          </>
                        )}
                      </NavLink>
