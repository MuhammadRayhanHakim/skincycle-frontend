          ...
          <div className="flex items-center gap-6 relative">
            ...
            <div className="flex items-center pl-5 border-l border-neutral-300">
              <div className="group relative cursor-pointer">
                {/* 🖼️ PERBAIKAN UTAMA: MENAMPILKAN FOTO PROFIL DINAMIS DARI DATABASE */}
                ...
                {/* ── PERBAIKAN BARIS DROPDOWN MENU USER (POJOK KANAN NAVBAR) ── */}
                <div className="absolute right-0 top-full pt-3 opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all pointer-events-none group-hover:pointer-events-auto z-50">
                  <div className="bg-neutral-default shadow-2xl rounded-2xl p-2 border border-neutral-100 min-w-[170px]">
                    {/* 👑 MENU 1: ADMIN PANEL (PERBAIKAN TOTAL STRUKTUR STRING CLASS) */}
                    {user.role === "admin" && (
                      <NavLink
                        to="/admin/dashboard"
                        className={({ isActive }) =>
                          `w-full text-left px-4 py-2 text-[11px] font-bold rounded-xl uppercase flex items-center gap-2 mb-1 transition-colors ${
                            isActive
                              ? "bg-[#3D5532] text-white hover:bg-[#3D5532]" // 🟢 Saat Aktif: Hijau tua, teks putih
                              : "text-neutral-600 hover:bg-neutral-50" // ⚪ Saat Tidak Aktif: Putih bersih, teks abu-abu (Sama seperti Akun Saya)
                          }`
                        }
                      >
                        {({ isActive }) => (
                          <>
                            <Settings
                              className={`w-3.5 h-3.5 ${isActive ? "text-white" : "text-brand-primary-300"}`}
                            />
                    ...
                    {/* 👤 MENU 2: AKUN SAYA (PERBAIKAN TOTAL STRUKTUR STRING CLASS) */}
                    <NavLink
                      to="/profil"
                      className={({ isActive }) =>
                        `w-full text-left px-4 py-2 text-[11px] font-bold rounded-xl uppercase flex items-center gap-2 mb-1 transition-colors ${
                          isActive
                            ? "bg-[#3D5532] text-white hover:bg-[#3D5532]" // Saat aktif: Hijau brand SkinCycle, teks putih
                            : "text-neutral-600 hover:bg-neutral-50" // Saat tidak aktif: Teks abu-abu bersih, hover latar belakang terang
                        }`
                      }
                    >
                      {({ isActive }) => (
