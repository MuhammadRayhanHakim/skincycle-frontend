...
//   // --- 1. REFAKTORISASI: TAMBAH KE KERANJANG SINKRON KE API DATABASE ---
...
//         alert(`📦 ${p.nama_produk} berhasil ditambahkan ke keranjang belanja!`);
//       } else {
//         alert("Gagal menambahkan ke keranjang: " + result.message);
//       }
//     } catch (error) {
//       console.error("Error network database keranjang:", error);
//       alert("Terjadi kesalahan jaringan saat menghubungi server.");
//     }
//   };

//   // --- 2. REFAKTORISASI: BELI SEKARANG LANGSUNG KE CHECKOUT ---
//   const handleBuyNow = (e, p) => {
//     e.stopPropagation();

//     if (!user) {
//       alert(
//         "⚠️ Akses Ditolak! Anda harus masuk (login) terlebih dahulu untuk melanjutkan proses transaksi.",
//       );
//       navigate("/masuk");
//       return;
//     }

//     // Memetakan ke format object flat agar bisa dibaca langsung oleh halaman checkout
//     const checkoutItem = [
//       {
//         id_produk: p.id_produk,
...
//           <h1 className="text-5xl font-marcellus mb-4 tracking-wide text-brand-dark-500">
//             Galeri Produk
//           </h1>
...
//               {["All", "Sunscreen", "Facial Wash", "Serum", "Moisturizer"].map(
//                 (cat) => (
//                   <label
//                     key={cat}
...
//                     <input
//                       type="checkbox"
//                       className="accent-brand-primary-300 w-4 h-4 rounded-md shadow-inner"
...
//                       <button
//                         type="button"
//                         onClick={(e) => handleAddCartDatabase(e, p)}
...
// 🌟 REVISI: Destructuring prop 'triggerToast' agar bisa memanggil Toast modern gelap SkinCycle
const ProductPage = ({ user, triggerToast }) => {
  const navigate = useNavigate();
  const [activeType, setActiveType] = useState("Semua");
  const [selectedCategories, setSelectedCategories] = useState(["All"]);
  const [products, setProducts] = useState([]);
  ...
  const handleAddCartDatabase = async (e, p) => {
    ...
    if (!user) {
      if (triggerToast) {
        triggerToast(
          "⚠️ Akses Ditolak! Silakan masuk ke akun Anda terlebih dahulu.",
        );
      }
      navigate("/masuk");
      return;
    ...
    try {
      ...
      const result = await response.json();
      if (response.ok || result.status === "success") {
        // 🌟 REVISI: Mengganti alert kaku menjadi Custom Toast Notification premium
        if (triggerToast) {
          triggerToast(
        ...
        if (triggerToast) {
          triggerToast("Gagal menambahkan ke keranjang: " + result.message);
        }
      }
    } catch (error) {
      console.error("Error network database keranjang:", error);
      if (triggerToast) {
        triggerToast("Terjadi kesalahan jaringan saat menghubungi server.");
      }
    }
  };
