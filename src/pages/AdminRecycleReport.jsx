...
// // 🚀 1. Integrasi Modul Peta Interaktif Leaflet Map Container
...
// // Fix konfigurasi asset path marker ikon Leaflet agar muncul sempurna dan tidak crash
...
//         // 🔥 FIX SINKRONISASI AKTIF: Jika sedang melihat detail laporan, update datanya secara real-time dari response terbaru
//         if (selectedLaporan) {
//           const dataTerbaru = result.data.find(
//             (item) => item.id_laporan === selectedLaporan.id_laporan,
//           );
//           if (dataTerbaru) {
//             setSelectedLaporan(dataTerbaru);
//           }
//         } else if (result.data.length > 0 && !selectedLaporan) {
//           // Default select antrean pertama yang belum selesai
...
//   // --- [A] KLIK TOMBOL: SELESAIKAN VERIFIKASI ---
...
//         // 🚀 FIX UTAMA (TANPA REFRESH): Update state selectedLaporan & list laporan secara instan di frontend
...
//         // Ambil data ulang dari server sebagai konfirmasi pendukung background
//         await fetchLaporan();
//       } else {
...
//   // --- [B] KLIK TOMBOL: KIRIM SALDO FINAL ---
...
//         setSelectedLaporan(null); // Bersihkan panel kanan karena proses selesai dan pindah ke tabel bawah
//         await fetchLaporan();
//       } else {
...
//   // --- AUTO-REFRESH SETIAP 30 DETIK (POLLING BACKGROUND) ---
//   useEffect(() => {
...
//   // Saringan data antrean aktif & arsip selesai
...
//                   <div>
//                     {/* 🚀 PERBAIKAN 1: Mengubah Judul Utama & Menyematkan Status Lencana di Atas Sesuai Gambar */}
//                     <div className="flex justify-between items-center mb-8">
//                       <h2 className="text-xl font-sans font-bold text-brand-dark-500">
...
//                     {/* Informasi Lapisan Form Konten Utama */}
...
//                       {/* 🚀 PERBAIKAN 2: Render parameter Berat Sampah Bersih Berdampingan Tanpa Card Box */}
//                       <div className="flex gap-3 items-start">
...
//                 {/* SISI KANAN PANEL VISUAL BUKTI FOTO & TOMBOL AKSI OPERASIONAL */}
...
//                         />
//                       ) : (
//                         <div className="w-full h-full flex items-center justify-center text-neutral-300 italic text-[11px]">
...
//                   {/* 🚀 PERBAIKAN 3: Memindahkan 2 Tombol Utama Ke Bagian Bawah Gambar Bukti Visual */}
...
//                           isActionLoading
//                         }
//                         className="flex-1 bg-[#3D5532] hover:bg-[#2b3c23] text-white py-3 px-4 rounded-xl font-bold text-xs flex items-center justify-center gap-1.5 shadow transition-all outline-none disabled:opacity-40 disabled:bg-neutral-100 disabled:text-neutral-400 disabled:cursor-not-allowed"
//                       >
...
//                           isActionLoading
//                         }
//                         className="flex-1 bg-white hover:bg-neutral-50 border border-[#3D5532] text-[#3D5532] py-3 px-4 rounded-xl font-bold text-xs flex items-center justify-center gap-1.5 shadow transition-all outline-none disabled:opacity-30 disabled:cursor-not-allowed"
//                       >
...
//         {/* MODAL CANVA DIALOG POPUP BALANCED REWARD */}
...
// Perbaikan asset path marker ikon Leaflet agar muncul di canvas admin
import markerIcon from "leaflet/dist/images/marker-icon.png";
import markerShadow from "leaflet/dist/images/marker-shadow.png";
const DefaultIcon = L.icon({
  iconUrl: markerIcon,
...
// Komponen pemicu pergeseran peta otomatis sewaktu admin mengganti baris antrean list pengguna
...
const AdminRecycleReport = () => {
  ...
  const handleKirimSaldoFinal = async (e) => {
    ...
    if (!isKonfirmasiPengepul)
      return alert("Aktifkan saklar konfirmasi pengepul.");
  ...
  // Konversi tipe string database ke objek float valid untuk Leaflet Canvas Container
    ...
    <div className="flex min-h-screen bg-neutral-50 font-sans relative text-brand-dark-500">
      ...
      <main className="flex-1 ml-64 p-8 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-10 items-start">
                ...
                antreanAktif.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center mt-2">
                      ...
                      <span
                        className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-md ${item.status_jemput === "sedang_dijemput" ? "bg-blue-50 text-blue-500 border border-blue-100" : "bg-feedback-warning-100 text-feedback-warning-300"}`}
                      >
                        {item.status_jemput?.replace("_", " ")}
                      </span>
                    </div>
          ...
          {/* ── 🚀 MONITOR PANEL DETAIL VERIFIKASI SEBELAH KANAN (PERBAIKAN TOTAL LAYOUT SAMA PERSIS GAMBAR TARGET) ── */}
          {/* ── MONITOR PANEL DETAIL VERIFIKASI (KANAN) ── */}
          <div className="lg:col-span-8 bg-neutral-default rounded-[45px] shadow-sm border border-neutral-100 h-fit overflow-hidden">
            {selectedLaporan ? (
              <>
                {/* 🚀 HEADER UTAMA DENGAN BORDER LINE BAWAH SESUAI GAMBAR TARGET */}
                ...
                {/* 🚀 KONTEN UTAMA (DIBERIKAN PADDING TERPISAH SETELAH HEADER) */}
                <div className="p-10 flex flex-col md:flex-row gap-10">
                  <div className="flex-1 flex flex-col justify-between space-y-6">
                    <div>
                      {/* Informasi Profil Avatar Pengirim */}
                      <div className="flex items-center gap-4 mb-8">
                        <div className="w-12 h-12 bg-neutral-100 rounded-full flex items-center justify-center border border-neutral-200 overflow-hidden shrink-0">
                          <img
                            src={
                  ...
                  {/* SISI KANAN PANEL: BUKTI FOTO & TOMBOL AKSI */}
                  <div className="w-full md:w-72 shrink-0 flex flex-col justify-between space-y-6">
                    <div>
                      <p className="text-xs font-bold text-brand-dark-500 mb-3">
                        Bukti Foto Sampah
