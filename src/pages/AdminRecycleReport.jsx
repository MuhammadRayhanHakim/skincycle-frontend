...
//   Banknote,
//   MapPin,
//   Scale, // Digunakan untuk ikon berat asli timbangan
// } from "lucide-react";
...
//   const [beratAsliInput, setBeratAsliInput] = useState(""); // 🆕 State baru untuk menampung berat riil dari timbangan admin
...
//       return alert("Silakan masukkan berat fisik asli timbangan yang valid."); // 🆕 Validasi input berat
//     if (!saldoInput || parseInt(saldoInput) <= 0)
//       return alert("Nominal saldo tidak valid.");
...
//             berat_asli: `${beratAsliInput} kg`, // 🆕 Kirim nilai timbangan riil ke backend untuk menimpa estimasi
//           }),
//         },
//       );
//       const result = await response.json();
...
//         setShowModal(false);
//         setSaldoInput("");
//         setBeratAsliInput(""); // 🆕 Reset form input berat asli
...
//                           type="button"
//                           onClick={() => {
//                             // Sinkronkan input default dengan estimasi angka awal agar admin tidak perlu ngetik dari nol jika cocok
//                             const matchNum =
...
//         {/* MODAL POPUP FORM VERIFIKASI (SUDAH DITAMBAHKAN FORM BERAT TIMBANGAN ASLI) */}
...
//                   {/* 🆕 1. FORM INPUT BERAT ASLI TIMBANGAN SEBELUM INPUT SALDO */}
//                   <div className="mb-4">
...
//                   {/* 2. FORM INPUT NOMINAL SALDO RUPIAH */}
//                   <div className="mb-5">
//                     <label className="text-[10px] font-black text-neutral-400 uppercase tracking-wider block mb-2 flex items-center gap-1">
...
const AdminRecycleReport = () => {
  ...
  const handleKirimSaldoFinal = async (e) => {
    e.preventDefault();
    if (!beratAsliInput || parseFloat(beratAsliInput) <= 0)
      return alert("Silakan masukkan berat fisik asli timbangan yang valid.");
    if (!saldoInput || parseInt(saldoInput) <= 0)
      return alert("Nominal saldo tidak valid.");
    if (!isKonfirmasiPengepul)
      return alert("Aktifkan saklar konfirmasi pengepul.");
                ...
                {daftarLaporanSelesai.length > 0 ? (
                  daftarLaporanSelesai.map((item) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-4 font-bold text-neutral-500">
                        {/* 🚀 FIX: Menampilkan berat_asli timbangan riil Admin, bukan estimasi kasar user */}
