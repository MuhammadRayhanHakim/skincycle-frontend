...
//   // 🌟 ── INTERKONEKSI INTEGRASI GOOGLE OAUTH ──
//   const handleGoogleLoginSuccess = useGoogleLogin({
//     onSuccess: async (tokenResponse) => {
//       console.log("Google Token Acquired:", tokenResponse);
//       const baseUrl = import.meta.env.VITE_API_URL || "http://localhost:5000";

//       try {
//         // Mengirimkan access_token dari Google menuju endpoint backend kita
...
//           alert(
//             `Berhasil masuk via Google! Selamat datang, ${userData.username}! 🍃`,
//           );
...
//           }
//         } else {
//           alert(result.message || "Gagal melakukan autentikasi Google");
//         }
//       } catch (error) {
//         console.error("Google Auth Backend Error:", error);
//         alert("Gagal terhubung ke server saat memproses login Google.");
//       }
//     },
//     onError: (error) => {
//       console.error("Google Login Failed:", error);
//       alert("Proses login dengan Google dibatalkan atau mengalami kegagalan.");
//     },
//   });
...
// 🌟 REVISI: Menambahkan destructuring prop 'triggerToast' dari App.jsx
const LoginPage = ({ setUser, triggerToast }) => {
  const navigate = useNavigate();
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.ok && result.status === "success") {
        ...
        // 🌟 REVISI: Mengganti alert browser dengan global Custom Toast Sukses
        if (triggerToast) {
          triggerToast(`Selamat datang kembali, ${userData.username}! ✨`);
        }
        ...
        // 🌟 REVISI: Mengganti alert gagal dengan Custom Toast
        if (triggerToast) {
          triggerToast(result.message || "Email atau kata sandi salah.");
        }
      }
    } catch (error) {
      console.error("Login Error:", error);
      // 🌟 REVISI: Mengganti alert error dengan Custom Toast
      if (triggerToast) {
  ...
  // 🌟 INTERKONEKSI INTEGRASI GOOGLE OAUTH DENGAN TOAST
  const handleGoogleLoginSuccess = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      console.log("Google Token Acquired:", tokenResponse);
      const baseUrl = import.meta.env.VITE_API_URL || "http://localhost:5000";
      ...
      try {
        ...
        if (response.ok && result.status === "success") {
          ...
          // 🌟 REVISI: Mengganti alert Google sukses dengan global Custom Toast
          if (triggerToast) {
            triggerToast(
              `Berhasil masuk via Google! Selamat datang, ${userData.username}! 🍃`,
            );
          ...
          // 🌟 REVISI: Mengganti alert Google gagal dengan Custom Toast
          if (triggerToast) {
            triggerToast(
              result.message || "Gagal melakukan autentikasi Google.",
            );
          }
        }
      } catch (error) {
        ...
        if (triggerToast) {
          triggerToast(
            "Gagal terhubung ke server saat memproses login Google.",
          );
        }
      }
    },
    onError: (error) => {
      console.error("Google Login Failed:", error);
      // 🌟 REVISI: Mengganti alert Google cancel dengan Custom Toast
      if (triggerToast) {
        triggerToast(
          "Proses login dengan Google dibatalkan atau mengalami kegagalan.",
        );
      }
    },
  });
          ...
          <div className="max-w-md w-full mx-auto space-y-6 py-4">
                ...
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (triggerToast)
                        triggerToast(
                          "Fitur reset kata sandi dalam pengembangan.",
                        );
                    }}
                    className="text-[10px] font-medium text-blue-500 hover:underline outline-none"
                  >
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                type="button"
                onClick={() => {
                  if (triggerToast) triggerToast("OAuth Apple segera hadir.");
                }}
                className="flex items-center justify-center gap-2 border border-gray-200 rounded-xl py-2 px-3 text-[11px] font-bold text-gray-700 bg-white hover:bg-gray-50 transition-colors outline-none"
              >
                <img
