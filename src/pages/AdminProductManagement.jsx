...
const AdminProductManagement = () => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      // 🚀 SWEETALERT: Error fetch produk — sebelumnya diam tanpa notifikasi
      Swal.fire({
        title: "Koneksi Gagal",
        text: "Tidak dapat memuat data produk. Pastikan server backend aktif.",
        icon: "error",
  ...
  const handleDelete = async (id, nama) => {
    // 🚀 SWEETALERT: Mengganti window.confirm() kaku dengan dialog konfirmasi premium
    const konfirmasi = await Swal.fire({
      title: "Hapus Produk?",
      text: `Produk "${nama}" akan dihapus dari database secara permanen dan tidak dapat dikembalikan.`,
      icon: "warning",
    ...
    try {
      ...
      if (response.ok) {
        // 🚀 SWEETALERT: Mengganti alert("Produk berhasil dihapus!") dengan modal sukses
        Swal.fire({
          title: "Produk Dihapus!",
          text: `"${nama}" telah berhasil dihapus dari katalog.`,
          icon: "success",
          confirmButtonColor: "#3d5532",
          customClass: { popup: "rounded-[30px]" },
        });
        ...
        // 🚀 SWEETALERT: Jika server merespons tidak OK
        Swal.fire({
          title: "Gagal Menghapus",
          text: "Server tidak dapat memproses penghapusan produk ini.",
          icon: "error",
          confirmButtonColor: "#3d5532",
        });
      }
      ...
      console.error(error);
      // 🚀 SWEETALERT: Error koneksi saat hapus produk
      Swal.fire({
        title: "Koneksi Terputus",
        text: "Terjadi kesalahan saat menghapus produk. Pastikan server aktif.",
        icon: "error",
        confirmButtonColor: "#3d5532",
      });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok || result.status === "success") {
        // 🚀 SWEETALERT: Mengganti alert("Data produk berhasil disimpan...") dengan modal sukses
        // handleCloseModal dan fetchProducts dijalankan setelah tombol OK ditekan
        Swal.fire({
          title: isEditMode ? "Perubahan Tersimpan!" : "Produk Dipublikasikan!",
          text: isEditMode
            ? "Data produk berhasil diperbarui dalam katalog."
            : "Produk baru berhasil ditambahkan ke dalam katalog.",
          icon: "success",
          confirmButtonColor: "#3d5532",
        ...
        // 🚀 SWEETALERT: Mengganti alert("Gagal memproses: " + result.message)
        Swal.fire({
          title: "Gagal Memproses",
          text: result.message || "Terjadi kesalahan saat menyimpan produk.",
          icon: "error",
          confirmButtonColor: "#3d5532",
        });
      }
      ...
      console.error(error);
      // 🚀 SWEETALERT: Mengganti alert("Gagal terhubung ke server backend.")
      Swal.fire({
        title: "Koneksi Terputus",
        text: "Gagal terhubung ke server backend. Pastikan server aktif dan coba lagi.",
        icon: "error",
        confirmButtonColor: "#3d5532",
      });
    } finally {
    ...
    <div className="flex min-h-screen bg-[#f7f9f6] font-sans text-[#1e3316]">
      ...
      <main className="flex-1 ml-64 p-10">
        ...
        <div className="w-full max-w-2xl grid grid-cols-1 md:grid-cols-2 gap-6 mb-10 items-start">
          <div className="bg-white px-6 py-5 rounded-3xl border border-neutral-100 shadow-sm flex items-center justify-between min-h-25">
            <div>
              <p className="text-[10px] font-black text-neutral-400 uppercase tracking-wider">
                Total Produk
              </p>
              <p className="text-3xl font-black mt-0.5 text-brand-dark-500">
                {totalProduk}
              </p>
            </div>
          ...
          <div className="bg-white px-6 py-5 rounded-3xl border border-neutral-100 shadow-sm flex items-center justify-between min-h-[100px]">
            <div>
              <p className="text-[10px] font-black text-neutral-400 uppercase tracking-wider">
                Kategori Aktif
              </p>
            ...
            <div className="w-12 h-12 bg-amber-50 rounded-2xl flex items-center justify-center text-amber-600 shrink-0 ml-4 shadow-sm">
                    ...
                    <td className="px-8 py-5 text-center">
                      <div className="flex items-center justify-center gap-3">
                        <button
                          ...
                          className="p-1.5 text-neutral-400 hover:text-[#3D5532] transition-colors outline-none"
                        >
                        ...
                        <button
                          ...
                          className="p-1.5 text-neutral-400 hover:text-red-500 transition-colors outline-none"
                        >
